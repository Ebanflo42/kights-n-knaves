...
showSoln numCharacters names soln =
  let knights = L.map (\i -> names!i) $ L.filter (testBit soln) [0..numCharacters]
      knaves = L.map (\i -> names!i) $ L.filter (not . (testBit soln)) [0..numCharacters]
  in "  Knights: " L.++ (L.intercalate ", " knights)
       L.++ "\n  " L.++ "Knaves: " L.++ (L.intercalate ", " knights)
